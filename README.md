...
- **产品定义**：定义儿童听故事场景中“AI 主动提问”和“用户打断提问”两条核心路径，以及打断、回答、略过和续播规则；
- **教育内容框架**：将大语文、百科、文化、逻辑、情绪、想象力和表达七个设计维度落实为问题 schema、学习目标、脚手架和证据记录；
- **产品 Workflow**：设计故事节点、问题节点、Question Planner、Session Memory 和事实型家长报告之间的完整闭环；
- **系统架构**：设计 Backend 权威游标、双端统一协议、Provider Contract 和按句恢复状态机；
- **完整原型验证**：在原始原型中跑通 Streaming ASR、LLM 增量分句、并发 Streaming TTS、有序 PCM 音频发送、低延迟打断和恢复；
- **跨端体验**：实现 Web 和微信小程序的故事馆、互动播放器、状态反馈与报告页面；
- **AI Coding 交付**：借助 AI Coding 完成端到端原型、核心测试和公开作品集工程化；
- **公开版治理**：完成凭证与 AppID 脱敏、第三方素材清理、原创示例替换，并使用 Mock Provider 保证无密钥复现。

项目过程与取舍见 [项目复盘](docs/case-study.md)。

## 当前边界

- 公开仓库可复现双交互状态机、Question Planner、Session Memory、跨端界面和报告闭环；
- 原始完整原型已跑通真实 Streaming ASR、LLM 和 TTS 链路，但相关供应商 adapter、凭证、测试语音和原始日志未公开；
- 现有 PERF 样本可支持 LLM 首 token 和完整发送耗时结论，尚不足以支持严格的公网端到端 SLA；
- 浏览器语音合成的音色由操作系统决定；微信小程序演示用定时器模拟播放完成；
- Memory Layer 仅保存在服务端进程内，按 session 隔离，不提供跨进程持久化或跨会话画像；
